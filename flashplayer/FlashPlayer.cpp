...
void FlashPlayer::RebuildBuffer(HWND hWnd,LONG nWidth,LONG nHeight)
{
	CleanupBuffer();
...
void FlashPlayer::GetFlashSize(LONG* nWidth,LONG* nHeight)
{
	*nWidth = m_nWidth;
	*nHeight = m_nHeight;
}

void FlashPlayer::GetBitmapSize(LONG* nWidth,LONG* nHeight)
{
...
BITMAPINFO FlashPlayer::GetBitmapInfo()
{
	return m_BMPINFO;
}
...
bool FlashPlayer::StartAnimation(const std::string& theFileName,LONG nWidth,LONG nHeight,HWND hWnd,LONG nBmpWidth,LONG nBmpHeight)
{
	...
	if (nBmpWidth == 0 || nBmpHeight == 0)
	{
		nBmpWidth = nWidth;
		nBmpHeight = nHeight;
	}

	RebuildBuffer(hWnd,nBmpWidth,nBmpHeight);
...
bool FlashPlayer::GetFlashCommand(std::string& sCommand, std::string& sParam)
{
...
void FlashPlayer::FlashCommand(const std::string& sCommand, const std::string& sParam)
{
	FLASHCOMMAND cmd;
...
int FlashPlayer::GetPercentLoaded()
{
	long aPercent = -1;
	if (m_pFlashInterface != NULL)
...
void FlashPlayer::SetLoop(BOOL bLoop)
{
	if (m_pFlashInterface != NULL)
...
void FlashPlayer::MouseUp(int x, int y)
{
	LRESULT aResult;
	if(m_pWindowlessObject)
...
BOOL FlashPlayer::Render(HDC hDC,BOOL bClear,COLORREF col)
{
	if(!m_pFlashInterface)
		return false;
	...
	if (aViewObject != NULL)
	{
		...
		if (bClear)
		{
			...
			FillRect(hDC,&aFill,br);
			DeleteObject(br);
			/*FillRect(hDC,&aFill,(HBRUSH)GetStockObject(/ *LTGRAY_BRUSH* // *NULL_BRUSH* // *BLACK_BRUSH* /WHITE_BRUSH));*/
		}
...
LONG FlashPlayer::SetPlayTimes(int time)
{
	return 1;
}